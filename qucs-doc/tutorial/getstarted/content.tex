...
% Copyright (C) 2007 Juan Carlos Borrás <jcborras@gmail.com>
%
...
The following text gives an overview about the Qucs-S
software application, what it can be used for and how it 
can be used.

\medskip

Qucs-S is free software licensed under the General Public License (GPL).
It can be downloaded from \url{https://ra3xdh.github.io/}. The complete
source code is available.  Every user of the program is allowed
to modify it as long as changes are made public.
...
software.  Build instructions are given.  Binary packages are available 
for Windows, MacOS, and certain Linux distributions (e.g. Debian, Ubuntu,
SuSE, Fedora, OpenSUSE).

\medskip

Once the software has been successfully installed on your system you
can start it by entering the following command in a terminal 
\begin{verbatim}
  # qucs-s
\end{verbatim}

or by clicking the appropriate icon on your start menu or
desktop.  Qucs-S is a multi-lingual program.  So depending on your
system's language settings the Qucs-S graphical user interface (GUI)
appears in different languages.

\medskip

The following image shows the application once it has been started.

\begin{figure}[ht]
  \centering
  \includegraphics[width=1\linewidth]{start}
  \caption{Qucs-S application main screen}
  \label{fig:start}
\end{figure}
\FloatBarrier
...
Qucs-S consists of several standalone programs interacting with each
...
\item the backend simulation engines,
